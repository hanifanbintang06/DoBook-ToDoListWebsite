'use client'

import { Nut, CalendarBlank, List } from "@phosphor-icons/react";
import SideBarButton from "./SideBarButton";
import Link from "next/link";
import { usePathname } from "next/navigation";

export default function SideBar() {
    const pathName = usePathname();

    return(
        <div className="w-fit h-screen flex flex-col gap-4">
            <div className="w-68 h-full px-4 pt-4 pb-5 flex flex-col gap-6 bg-white rounded-2xl">
                <div className="w-auto h-fit px-4 py-3 flex flex-row gap-5">
                    <button className="cursor-pointer">
                        <List size={20} />
                    </button>
                    <div className="w-fit h-fit flex flex-row items-center gap-2">
                        <svg width="20px" height="100%" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M33.724 36.5809C37.7426 32.5622 40.0003 27.1118 40.0003 21.4286C40.0003 15.7454 37.7426 10.2949 33.724 6.27629C29.7054 2.25765 24.2549 1.02188e-06 18.5717 0C12.8885 -1.02188e-06 7.43807 2.25764 3.41943 6.27628L10.4905 13.3473C11.6063 14.4631 13.4081 14.4074 14.8276 13.7181C15.9836 13.1568 17.2622 12.8571 18.5717 12.8571C20.845 12.8571 23.0252 13.7602 24.6326 15.3677C26.2401 16.9751 27.1431 19.1553 27.1431 21.4286C27.1431 22.7381 26.8435 24.0167 26.2822 25.1727C25.5929 26.5922 25.5372 28.394 26.6529 29.5098L33.724 36.5809Z" fill="#297AFF"></path>
                        <path d="M30 40H19.5098C17.9943 40 16.5408 39.398 15.4692 38.3263L1.67368 24.5308C0.60204 23.4592 0 22.0057 0 20.4902V10L30 40Z" fill="#34C2FF"></path>
                        <path d="M10.7143 39.9999H4.28571C1.91878 39.9999 0 38.0812 0 35.7142V29.2856L10.7143 39.9999Z" fill="#34C2FF"></path>
                        </svg>
                        <h1 className="text-lg font-medium">Do Book</h1>
                    </div>
                </div>
                <div className="w-full h-full flex flex-col gap-5 overflow-y-auto">
                    <div className="w-auto h-fit gap-4 flex flex-col">
                        <h2 className="text-sm text-[#767676] px-5">Menu</h2>
                        <div className="w-auto h-fit flex flex-col">
                            <Link href={`/`}>
                                <SideBarButton active={pathName == `/`}>
                                    <Nut size={18} />
                                    <span>Home</span>
                                </SideBarButton>
                            </Link>
                            <SideBarButton active={false}>
                                <Nut size={18} />
                                <span>Recent</span>
                            </SideBarButton>
                            <SideBarButton active={false}>
                                <CalendarBlank size={18} />
                                <span>Calender</span>
                            </SideBarButton>
                        </div>
                    </div>
                    <div className="w-auto h-fit gap-4 flex flex-col">
                        <h2 className="text-sm text-[#767676] px-5">Note</h2>
                        <div className="w-auto h-fit flex flex-col">
                            <Link href={`/Notes`}>
                                <SideBarButton active={pathName == '/Notes'}>
                                    <Nut size={18} />
                                    <span>Notes</span>
                                </SideBarButton>
                            </Link>
                        </div>
                    </div>
                    <div className="w-auto h-fit gap-4 flex flex-col">
                        <h2 className="text-sm text-[#767676] px-5">Team</h2>
                        <div className="w-auto h-fit flex flex-col">
                            <SideBarButton active={false}>
                                <Nut size={18} />
                                <span>Colab</span>
                            </SideBarButton>
                        </div>
                    </div>
                </div>
            </div>
            <div className="w-full h-fit p-4 flex flex-row bg-white rounded-2xl">
                <div className="w-fit h-fit flex flex-col gap-1">
                    <p className="text-sm font-medium">Hanifan Bintang</p>
                    <p className="text-xs text-[#767676]">User</p>
                </div>
            </div>
        </div>
    )
}